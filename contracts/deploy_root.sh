...
# DEBOT_NAME3=${3%.*} # filename without extension
...
function giver {
    $tos --url $NETWORK call \
        --abi ./deploy/giver.abi.json \
        --sign ./deploy/giver.keys.json \
        $GIVER_ADDRESS \
...
tondev sol compile $ROOTTOKEN.sol
# tondev sol compile $DEBOT_NAME2.sol
# tondev sol compile $DEBOT_NAME3.sol
...
echo "Step 1. Calculating root token address"
genaddr $ROOTTOKEN
ROOTTOKEN_ADDRESS=$(get_address $ROOTTOKEN)

# genaddr_setkey $DEBOT_NAME2 $DEBOT_NAME
# DEBOT_ADDRESS2=$(get_address $DEBOT_NAME2)

# genaddr_setkey $DEBOT_NAME3 $DEBOT_NAME
# DEBOT_ADDRESS3=$(get_address $DEBOT_NAME3)
...
echo "Step 3. Sending tokens to ROOTTOKEN address: $ROOTTOKEN_ADDRESS"
giver $ROOTTOKEN_ADDRESS


# ################################################### Root public key взять из сгенерированных ключей и подставить ниже

echo "Step 4. Deploying ROOTTOKEN contract"
$tos --url $NETWORK deploy $ROOTTOKEN.tvc '{\"name\":\"656261436f696e\",\"symbol\":\"454243\", \"decimals\":\"0\",\"root_public_key\":\"0x<Root public key>\",\"wallet_code\":\"'$TVM_WALLET_CODE'\",\"total_supply\":\"<Number of tokens>\"}' \
    --sign $ROOTTOKEN.keys.json \
    --abi $ROOTTOKEN.abi.json 1>/dev/null
...
echo "Set storage, debot2, debot3 addreses to debot1"
$tos --url $NETWORK call $DEBOT_ADDRESS setAddreses "{\"storageAddress\":\"$STORAGE_ADDRESS\",\"wgBot_deployerAddr\":\"$DEBOT_ADDRESS2\",\"wgBot_UnitsAddr\":\"$DEBOT_ADDRESS3\"}" \
    --sign $DEBOT_NAME.keys.json \
    --abi $DEBOT_NAME.abi.json 1>/dev/null
echo "Success"
...
echo "Step 10. Sending tokens to debot3 address: $DEBOT_ADDRESS3"
giver $DEBOT_ADDRESS3

echo "Step 8. Deploying debot3"
$tos --url $NETWORK deploy $DEBOT_NAME3.tvc "{}" \
    --sign $DEBOT_NAME.keys.json \
    --abi $DEBOT_NAME3.abi.json 1>/dev/null

echo "Creating ABI"
DEBOT_ABI3=$(cat $DEBOT_NAME3.abi.json | xxd -ps -c 20000)
echo "Success"

echo "Set ABI"
$tos --url $NETWORK call $DEBOT_ADDRESS3 setABI "{\"dabi\":\"$DEBOT_ABI3\"}" \
    --sign $DEBOT_NAME.keys.json \
    --abi $DEBOT_NAME3.abi.json 1>/dev/null
echo "Success"
...
echo "Done! Deployed debot3 with address: $DEBOT_ADDRESS3"
